...
<body>
	<section class="menu_sec">
		...
		<div class="search_par">
			<input required placeholder="Search Here ..." id="search_bar"
				type="text" /> <a id="search_btn" href=""> <img
				width="35" src="search.png" alt="" />
			</a>
		</div>
	</section>
	...
	<section class="news_par">
		...
		DonorRequest dReq = drDao.getRequestByRequestId(Integer.parseInt(request.getParameter("req")));
		%>


		<form action="UpdateRequestServlet" id="create_request_form" method="post">
	...
	<script>
	document.getElementById("patient_name").value="<%=dReq.getPatient_name()%>";
	document.getElementById("why").value="<%=dReq.getWhy_need()%>";
	document.getElementById("need").value="<%=dReq.getBlood_unit()%>";
	document.getElementById("mobile").value="<%=dReq.getMobile()%>";
	document.getElementById("location").value="<%=dReq.getLocation()%>";
	document.getElementById("group").value="<%=dReq.getBlood_group()%>";
	</script>
</body>
</html>
